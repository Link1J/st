...
void xdrawglyphfontspecs(XftGlyphFontSpec const* specs, Glyph base, int len, int x, int y)
{
    ...
    if (base.mode & ATTR_UNDERLINE)
    {
        ...
        XGCValues ugcv = {.foreground = linecolor, .line_width = wlw, .line_style = LineSolid, .cap_style = CapNotLast};
        ...
        else if (base.ustyle == 3)
        {
            ...
            */
            ...
            if (npoints >= 3)
            {
                ...
                points[0] = (XPoint){.x = wx + marginStart, .y = (isSlopeRising(wx, 0, ww)) ? (wy - marginStart + ww / 2.f) : (wy + marginStart)};

                // Second point (Goes back to the absolute point coordinates)
                points[1] = (XPoint){.x = (ww / 2.f) - marginStart, .y = (isSlopeRising(wx, 1, ww)) ? (ww / 2.f - marginStart) : (-ww / 2.f + marginStart)};
                waveLength += (ww / 2.f) - marginStart;

                // The rest of the points
                for (int i = 2; i < npoints - 1; i++)
                {
                    points[i] = (XPoint){.x = ww / 2, .y = (isSlopeRising(wx, i, ww)) ? wh / 2 : -wh / 2};
                    waveLength += ww / 2;
                }

                // Last point
                points[npoints - 1] = (XPoint){.x = ww / 2, .y = (isSlopeRising(wx, npoints - 1, ww)) ? wh / 2 : -wh / 2};
                waveLength += ww / 2;
                ...
                { // Add a bonus point?
                    ...
                    points[npoints] = (XPoint){.x = marginEnd, .y = (isSlopeRising(wx, npoints, ww)) ? (marginEnd) : (-marginEnd)};
